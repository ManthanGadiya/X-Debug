...
* **Completed:**
  ...
  * Dev-workflow fixes: the backend quick start excludes `.xdebug-workspace` from uvicorn's reload watch so cloning from GitHub no longer triggers a server restart (which had wiped in-memory projects and caused a 404), and frontend polling pauses while the browser tab is hidden so background tabs stop hammering the API (`GET /projects`, `/analysis`, `/runtime`, `/tests`, `/health`) — 54 vitest passed, typecheck and lint clean, merged to `main` via PR #11
