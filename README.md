...
* **Milestone:** Phase 9 — Testing & Optimization — backend runtime coverage at 100%, over-engineering audit, parse caching, parallel parsing, and dev-workflow bug fixes complete
* **Branch:** `main`
* **Completed:**
  ...
  * Dev-workflow fixes: the backend quick start excludes `.xdebug-workspace` from uvicorn's reload watch and the workspace now defaults to the repo root (outside the watched `backend/` tree), so cloning from GitHub no longer triggers a server restart (which had wiped in-memory projects and caused a 404), and frontend polling pauses while the browser tab is hidden so background tabs stop hammering the API (`GET /projects`, `/analysis`, `/runtime`, `/tests`, `/health`) — 54 vitest passed, typecheck and lint clean; 325 pytest at 96.36% coverage, ruff/black/mypy clean, merged to `main` via PRs #11 and #13
...
The API runs at **http://localhost:8000** with interactive docs at **http://localhost:8000/docs**. No configuration is required — the backend boots with safe defaults and writes analysis artifacts under the repo-root `.xdebug-workspace` directory (outside the reload-watched `backend/` tree).
